...
function renderBoard(data, options = {}) {
  ...
  for (const t of data.tickets || []) {
    if (t.pendingAction) {
      const source = t.pendingAction.metadata?.source || "";
      if (QUESTION_SOURCES.has(source)) {
        pendingQuestions.push(t);
      } else {
        pendingActions.push(t);
      }
    } else if (t.status === "review") {
      reviewTickets.push(t);
    } else if (ACTIVE_STATUSES.has(t.status)) {
      activeTickets.push(t);
    } else if (INACTIVE_STATUSES.has(t.status)) {
      inactiveTickets.push(t);
    }
  ...
  const allPending = [...pendingQuestions, ...pendingActions, ...reviewTickets, ...activeTickets, ...inactiveTickets];
  ...
  const stats = [
    ...
    `${c.yellow}? ${pendingQuestions.length} question${pendingQuestions.length !== 1 ? "s" : ""}${c.reset}`,
    `${c.magenta}\u25cf ${pendingActions.length} action${pendingActions.length !== 1 ? "s" : ""}${c.reset}`,
    `${c.cyan}\u25cb ${reviewTickets.length} review${c.reset}`,
    `${c.red}\u25a0 ${inactiveTickets.length} inactive${c.reset}`,
  ...
  // Review - completed steps awaiting user confirmation to advance
  if (reviewTickets.length) {
    out.push(hline(termWidth - 2, `Review (${reviewTickets.length})`));
    for (const t of reviewTickets) {
      const isCursorT = focus === "tickets" && ticketCursor >= 0 && allTickets[ticketCursor]?.id === t.id;
      const globalIdx = allPending.indexOf(t);
      const isCursorP = focus === "pending" && globalIdx === pendingCursor;
      const isCursor = isCursorT || isCursorP;
      const prefix = isCursor ? `${c.cyan}\u25b6${c.reset} ` : "  ";
      const skill = safe(t.currentSkill);
      out.push(`${prefix}${c.cyan}\u25cb${c.reset} ${c.bold}#${t.id}${c.reset} ${safe(t.title)} ${c.dim}(${skill} step ${safe(t.currentStep)} complete)${c.reset}  ${c.cyan}n${c.reset}${c.dim}: next${c.reset}`);
    }
    out.push("");
  }

  // Active (running/queued) - for stop/delete access from pending focus
  if (activeTickets.length) {
    out.push(hline(termWidth - 2, `Active (${activeTickets.length})`));
    for (const t of activeTickets) {
      ...
      const isCursor = isCursorT || isCursorP;
      const prefix = isCursor ? `${c.cyan}\u25b6${c.reset} ` : "  ";
      out.push(`${prefix}${statusBadge(t.status)} ${c.bold}#${t.id}${c.reset} ${safe(t.title)} ${c.dim}(${safe(t.currentSkill)} step ${safe(t.currentStep)})${c.reset}  ${c.reset}${c.bold}s${c.reset}${c.dim}: stop  ${c.reset}${c.bold}d${c.reset}${c.dim}: delete${c.reset}`);
    }
    out.push("");
  }

  // Inactive (halted, blocked, awaiting_input without pendingAction)
  if (inactiveTickets.length) {
    out.push(hline(termWidth - 2, `Inactive (${inactiveTickets.length})`));
    for (const t of inactiveTickets) {
      ...
      const isCursor = isCursorT || isCursorP;
      const prefix = isCursor ? `${c.cyan}\u25b6${c.reset} ` : "  ";
      out.push(`${prefix}${statusBadge(t.status)} ${c.bold}#${t.id}${c.reset} ${safe(t.title)} ${c.dim}(${safe(t.currentSkill)} step ${safe(t.currentStep)})${c.reset}  ${c.reset}${c.bold}R${c.reset}${c.dim}: retry  ${c.reset}${c.bold}n${c.reset}${c.dim}: next  ${c.reset}${c.bold}d${c.reset}${c.dim}: delete${c.reset}`);
    }
    out.push("");
  }
  ...
  if (done.length) {
    ...
    for (const t of done.slice(0, 10)) {
      const isCursor = focus === "tickets" && ticketCursor >= 0 && allTickets[ticketCursor]?.id === t.id;
      const prefix = isCursor ? `${c.cyan}\u25b6${c.reset} ` : "  ";
      out.push(`${prefix}${c.green}\u2713${c.reset} ${c.dim}#${t.id}${c.reset} ${safe(t.title)}`);
    }
    out.push("");
  }

  // Pending Questions (real user questions from tool_use / runtime_fallback)
  out.push(hline(termWidth - 2, `Questions (${pendingQuestions.length})`));
  if (!pendingQuestions.length) {
    ...
    for (let i = 0; i < pendingQuestions.length; i += 1) {
      ...
      const firstLine = prompt.split("\n")[0];
      const globalIdx = allPending.indexOf(t);
      const isActive = focus === "pending" && globalIdx === pendingCursor;
      if (isActive) {
        out.push(`  ${c.cyan}${c.bold}\u25b6 ${i + 1}.${c.reset} ${c.bold}#${t.id}${c.reset} ${safe(t.title)}`);
        out.push(`    ${c.yellow}? ${firstLine}${c.reset}`);
      } else {
        out.push(`  ${c.dim}  ${i + 1}.${c.reset} ${c.dim}#${t.id}${c.reset} ${safe(t.title)}`);
  ...
  // Pending Actions (recovery, approval, system)
  if (pendingActions.length) {
    out.push(hline(termWidth - 2, `Actions (${pendingActions.length})`));
    for (let i = 0; i < pendingActions.length; i += 1) {
      const t = pendingActions[i];
      const prompt = resolvePrompt(t);
      const firstLine = prompt.split("\n")[0];
      const globalIdx = allPending.indexOf(t);
      const isActive = focus === "pending" && globalIdx === pendingCursor;
      const source = t.pendingAction?.metadata?.source || "";
      const tag = source === "recovery" ? `${c.red}\u25a0${c.reset}` : `${c.magenta}\u25cf${c.reset}`;
      if (isActive) {
        out.push(`  ${c.cyan}${c.bold}\u25b6${c.reset} ${tag} ${c.bold}#${t.id}${c.reset} ${safe(t.title)}`);
        out.push(`    ${c.magenta}${firstLine}${c.reset}`);
      } else {
        out.push(`  ${c.dim} ${c.reset} ${tag} ${c.dim}#${t.id}${c.reset} ${safe(t.title)}`);
        out.push(`    ${c.dim}${firstLine}${c.reset}`);
      }
    }
    out.push("");
  ...
  const keys = focus === "tickets"
    ? `${c.dim}\u2191\u2193 move  ${k("n","ext")}  ${k("s","top")}  ${k("d","elete")}  ${k("a","dd")}  ${k("l","og")}  ${k("c","lear done")}  ${c.dim}Enter answer  ${k("r","efresh")}  ${k("q","uit")}`
    : `${c.dim}\u2191\u2193 move  1-9 jump  ${c.dim}Enter answer  ${k("n","ext")}  ${k("s","top")}  ${k("d","elete")}  ${k("a","dd")}  ${k("l","og")}  ${k("c","lear done")}  ${k("r","efresh")}  ${k("q","uit")}`;
  out.push(`  ${keys}`);
