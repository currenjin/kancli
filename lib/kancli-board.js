...
function renderTicketCard(t, cardWidth, isCursor) {
  ...
  const age = timeAgo(t.updatedAt || t.createdAt);
  const step = `${c.dim}step ${safe(t.currentStep)}${age ? `  ${age} ago` : ""}${c.reset}`;
...
function renderBoard(data, options = {}) {
  ...
  if (reviewTickets.length) {
    ...
    for (const t of reviewTickets) {
      ...
      out.push(`${prefix}${c.cyan}\u25cb${c.reset} ${c.bold}#${t.id}${c.reset} ${safe(t.jiraTicket)} ${c.dim}(${skill} step ${safe(t.currentStep)} complete)${c.reset}  ${c.cyan}n${c.reset}${c.dim}: next${c.reset}`);
    }
    out.push("");
  }
  ...
  if (activeTickets.length) {
    ...
    for (const t of activeTickets) {
      ...
      const isCursor = isCursorT || isCursorP;
      const prefix = isCursor ? `${c.cyan}\u25b6${c.reset} ` : "  ";
      out.push(`${prefix}${statusBadge(t.status)} ${c.bold}#${t.id}${c.reset} ${safe(t.jiraTicket)} ${c.dim}(${safe(t.currentSkill)} step ${safe(t.currentStep)})${c.reset}  ${c.reset}${c.bold}s${c.reset}${c.dim}: stop  ${c.reset}${c.bold}d${c.reset}${c.dim}: delete${c.reset}`);
    }
    out.push("");
  }
  ...
  if (pendingActions.length) {
    ...
    for (let i = 0; i < pendingActions.length; i += 1) {
      ...
      if (isActive) {
        out.push(`  ${c.cyan}${c.bold}\u25b6${c.reset} ${tag} ${c.bold}#${t.id}${c.reset} ${safe(t.jiraTicket)}`);
        out.push(`    ${c.magenta}${firstLine}${c.reset}`);
      } else {
        out.push(`  ${c.dim} ${c.reset} ${tag} ${c.dim}#${t.id}${c.reset} ${safe(t.jiraTicket)}`);
        out.push(`    ${c.dim}${firstLine}${c.reset}`);
      }
    }
    out.push("");
