...
const STATUS_STYLE = {
  ...
  waiting:        { symbol: "\u25cf", color: c.yellow,  label: "waiting" },
...
function renderBoard(data, options = {}) {
  ...
  const allPending = [...waitingTickets, ...activeTickets, ...inactiveTickets];
  ...
  const stats = [
    ...
    `${c.yellow}\u25cf ${waitingTickets.length} waiting${c.reset}`,
  ...
  // Waiting (all tickets with pendingAction)
  out.push(hline(termWidth - 2, `Waiting (${waitingTickets.length})`));
  if (!waitingTickets.length) {
    out.push(`  ${c.dim}(no waiting tickets)${c.reset}`);
  } else {
    for (let i = 0; i < waitingTickets.length; i += 1) {
